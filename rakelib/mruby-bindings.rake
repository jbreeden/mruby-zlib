...
namespace :bindings do
  ...
  task :scrape do
    ...
    headers.each do |header|
      sh "clang2json #{header} | egrep -v 'FunctionDecl.*\"name\": \"(gzclose_w|gzclose_r|[^\"]+_\")' >> declarations.json"
    end
  end
  ...
  task :'enable-functions' do
    sh 'mrbind enable-functions -m ZLib -g mruby-zlib -o .'
  end
  
  task :fn_count do
    sh "cat include/mruby_ZLib_functions.h | egrep 'TRUE|FALSE' | wc -l"
  end

  task :bound_fns do
    sh "cat include/mruby_ZLib_functions.h | egrep 'TRUE'"
  end

  task :bound_fn_count do
    sh "cat include/mruby_ZLib_functions.h | egrep 'TRUE' | wc -l"
  end

  task :unbound_fns do
    sh "cat include/mruby_ZLib_functions.h | egrep 'FALSE'"
  end

  task :unbound_fn_count do
    sh "cat include/mruby_ZLib_functions.h | egrep 'FALSE' | wc -l"
  end
end
